...
class DataIngestion:
    ...
    def apply_data_ingestion(self):
        ...
        try:
            ...
            # Renaming the columns for better data manipulation, interpretation and modelling.
            train.columns = [x.lower().replace(' ', '_') for x in train.columns]
            train.rename(columns={'risk': 'default'}, inplace=True)
            
            test.columns = [x.lower().replace(' ', '_') for x in test.columns]
            test.rename(columns={'risk': 'default'}, inplace=True)
